...
export default function Auth(props: any) {
  ...
  const _cloneElement = cloneElement(props.children, {
    btns: meta.btns && userInfo ? meta.btns[userInfo.auth] : [],
  })
  // 测试
...
export default function Aaa(props: any) {
  console.log(props, 'props bbbb')

  return <div>
   {props.btns.includes("look") && <button>add</button>}
        {props.btns.includes("change") && <button>delete</button>}
        {props.btns.includes("delete") && <button>edit</button>}
        {props.btns.includes("stop") && <button>select</button>}
  </div>
}
```
